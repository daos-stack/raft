    ...
    )
{
    ...
    {
        err = raft_set_current_term(me_, ae->term);
        if (0 != err)
            goto out;
        raft_become_follower(me_);
    }
    ...
    if (0 < ae->prev_log_idx)
    {
        ...
        if (e->term != ae->prev_log_term)
        {
            ...
            err = raft_delete_entry_from_idx(me_, ae->prev_log_idx);
            goto out;
        }
    }
    ...
    for (i = 0; i < ae->n_entries; i++)
    {
        ...
        {
            err = raft_delete_entry_from_idx(me_, ety_index);
            if (0 != err)
                goto out;
            break;
        }
    ...
    {
        err = raft_append_entry(me_, &ae->entries[i]);
        if (0 != err)
            goto out;
                    ...
                    msg_entry_response_t *r)
{
    ...
    int err = raft_append_entry(me_, e);
    if (0 != err)
        return err;
    for (i = 0; i < me->num_nodes; i++)
    {
...
int raft_send_requestvote(raft_server_t* me_, raft_node_t* node)
{
    ...
    if (me->cb.send_requestvote)
        err = me->cb.send_requestvote(me_, me->udata, node, &rv);
    return err;
}
